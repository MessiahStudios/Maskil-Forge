...
app.MapPost("/api/host/vst3-native-factory-check", async (Vst3NativeCheckRequest request, Vst3NativeCheck nativeCheck, HttpContext context, CancellationToken cancellationToken) =>
{
    context.Response.Headers.CacheControl = "no-store";
    if (!NativePluginAccess.IsLocalRequest(context)) return Results.StatusCode(StatusCodes.Status403Forbidden);
...
app.MapGet("/api/instrument-midi-portamentos", () => Results.Ok(InstrumentMidiPortamentoMapper.Map()));
...
static void ApplyRequest(ProjectEditor editor, ProjectCommandRequest request)
{
    ...
    switch (request.Type.Trim().ToLowerInvariant())
    {
        ...
        case "accept-vocal-saturation": editor.Execute(new AcceptVocalSaturationCommand(
            RequiredAssetId(request), request.SourceSha256 ?? throw new ArgumentException("The previewed source digest is required."))); break;
        case "accept-vocal-character-compression": editor.Execute(new AcceptVocalCharacterCompressionCommand(
            RequiredAssetId(request), request.SourceSha256 ?? throw new ArgumentException("The previewed source digest is required."))); break;
