...
public sealed class AcceptVocalSibilanceCommand(ProjectAssetId assetId, string sourceSha256) : IProjectCommand
{
    private VocalProcessingRecipe? _previous;
    private VocalProcessingRecipe? _accepted;
    ...
    public void Execute(SongProject project)
    {
        var recipe = _accepted ?? VocalProcessingRecipe.FixedSibilance(assetId, sourceSha256, DateTimeOffset.UtcNow);
        var previous = project.VocalProcessingRecipes.SingleOrDefault(item =>
            item.AssetId == assetId && item.Role == VocalProcessingRole.SibilanceControl);
        project.SetVocalProcessingRecipe(recipe);
        if (_accepted is null) _previous = previous;
        _accepted = recipe;
    }
    ...
    {
        if (_accepted is null) throw new InvalidOperationException("Command has not been executed.");
        if (_previous is null) project.ClearVocalProcessingRecipe(assetId, VocalProcessingRole.SibilanceControl);
        else project.SetVocalProcessingRecipe(_previous);
    }
}

public sealed class AcceptVocalSpaceCommand(ProjectAssetId assetId, string sourceSha256) : IProjectCommand
{
    private VocalProcessingRecipe? _previous;
    private VocalProcessingRecipe? _accepted;
    ...
    public void Execute(SongProject project)
    {
        var recipe = _accepted ?? VocalProcessingRecipe.FixedSpace(assetId, sourceSha256, DateTimeOffset.UtcNow);
        var previous = project.VocalProcessingRecipes.SingleOrDefault(item =>
            item.AssetId == assetId && item.Role == VocalProcessingRole.Space);
        project.SetVocalProcessingRecipe(recipe);
        if (_accepted is null) _previous = previous;
        _accepted = recipe;
    }
    ...
    {
        if (_accepted is null) throw new InvalidOperationException("Command has not been executed.");
        if (_previous is null) project.ClearVocalProcessingRecipe(assetId, VocalProcessingRole.Space);
        else project.SetVocalProcessingRecipe(_previous);
    }
}
