...
export const vocalLevelControl = Object.freeze({
  processorId: 'maskil.vocal.level-control.v1',
  role: 'TransparentDynamics',
  thresholdDecibels: -18,
  ratio: 2,
  attackMilliseconds: 20,
  releaseMilliseconds: 120,
})
...
function channelsOf(buffer) {
  ...
  if (!Number.isInteger(sampleRate) || sampleRate < 8000 || sampleRate > 96000 ||
    ...
    throw new Error('Level-control preview supports up to one minute of mono or stereo audio at 8–96 kHz.')
...
export function renderVocalLevelControl(buffer, settings = vocalLevelControl) {
  if (settings?.thresholdDecibels !== vocalLevelControl.thresholdDecibels || settings.ratio !== vocalLevelControl.ratio ||
      settings.attackMilliseconds !== vocalLevelControl.attackMilliseconds || settings.releaseMilliseconds !== vocalLevelControl.releaseMilliseconds)
    throw new Error('This preview uses the fixed level-control starting point.')
  ...
  for (let index = 0; index < buffer.length; index++) {
    ...
    if (!Number.isFinite(gain) || gain <= 0 || gain > 1)
      throw new Error('This level-control preview would exceed available headroom. No limiting or makeup gain was applied.')
...
export async function prepareVocalLevelControl(url, asset, signal, environment = globalThis) {
  ...
  try {
    ...
    abort()
    const processed = renderVocalLevelControl(buffer)
    ...
    abort()
    let disposed = false
    return { originalUrl, processedUrl, settings: vocalLevelControl, dispose() {
      if (disposed) return
      disposed = true
