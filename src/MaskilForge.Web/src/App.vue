        ...
        <article v-for="asset in instrumentRetargetTakes" :key="asset.id" class="harmony-note-sketch" :aria-label="`Catalog instrument retarget for ${asset.name}`">
          ...
          <div v-if="instrumentPerformanceSketches[asset.id]" class="harmony-note-sketch-result">
            <p>
              ...
              <span>Uses the first tempo only. Each instrument keeps its own catalog technique. Piano, bass, flute, clarinet, and trumpet slides stay unused. Drum-kit swell and slide stay unused. Pitched instruments do not take kit hits. Kit hits use Acoustic Bass Drum at C2. Named pitched parts use inspectable MIDI channels and an inspectable General MIDI program on named format-1 tracks. The conductor track includes the song key as a MIDI key signature, each stored section title as a MIDI marker, each placed syllable as a MIDI lyric, and each stored harmony chord as MIDI text. Tagged dynamics use the instrument’s inspectable controller; flute swell is CC 2 and synth lead swell is CC 74; other catalog swells stay CC 11. Cello, violin, acoustic guitar, and electric guitar declare ±2 semitones of pitch-bend range without moving the wheel; synth-lead portamento is CC 65 and stays off so stored notes stay discrete. Drum kit stays on channel 10 without a program change, a dynamics controller, or a pitch-bend range; unassigned notes stay on channel 1; untagged dynamics stay CC 11.</span>
            </p>
