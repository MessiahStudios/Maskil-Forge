...
const instrumentMidiPrograms = ref<InstrumentMidiProgramMapSet | null>(null)
const instrumentMidiControllers = ref<InstrumentMidiControllerMapSet | null>(null)
const instrumentMidiPitchBends = ref<InstrumentMidiPitchBendMapSet | null>(null)
const instrumentMidiPortamentos = ref<InstrumentMidiPortamentoMapSet | null>(null)
...
async function refreshWorkspaceHealth() {
  ...
  try {
    ...
    try {
      instrumentMidiPrograms.value = await projectsApi.instrumentMidiPrograms()
      activityLog.write('info', 'instrument-midi-programs.load', 'Instrument MIDI programs loaded.', {
        mappedCount: instrumentMidiPrograms.value.assignments.filter(item => item.applicable).length,
        notApplicableCount: instrumentMidiPrograms.value.assignments.filter(item => !item.applicable).length,
      })
    } catch (error) {
      instrumentMidiPrograms.value = null
      activityLog.write('warning', 'instrument-midi-programs.load', error instanceof Error ? error.message : 'Instrument MIDI programs could not be loaded.')
    }
    try {
      instrumentMidiControllers.value = await projectsApi.instrumentMidiControllers()
      activityLog.write('info', 'instrument-midi-controllers.load', 'Instrument MIDI controllers loaded.', {
        mappedCount: instrumentMidiControllers.value.assignments.filter(item => item.applicable).length,
        notApplicableCount: instrumentMidiControllers.value.assignments.filter(item => !item.applicable).length,
      })
    } catch (error) {
      instrumentMidiControllers.value = null
      activityLog.write('warning', 'instrument-midi-controllers.load', error instanceof Error ? error.message : 'Instrument MIDI controllers could not be loaded.')
    }
    try {
      instrumentMidiPitchBends.value = await projectsApi.instrumentMidiPitchBends()
      activityLog.write('info', 'instrument-midi-pitch-bends.load', 'Instrument MIDI pitch-bend ranges loaded.', {
        mappedCount: instrumentMidiPitchBends.value.assignments.filter(item => item.applicable).length,
        notApplicableCount: instrumentMidiPitchBends.value.assignments.filter(item => !item.applicable).length,
      })
    } catch (error) {
      instrumentMidiPitchBends.value = null
      activityLog.write('warning', 'instrument-midi-pitch-bends.load', error instanceof Error ? error.message : 'Instrument MIDI pitch-bend ranges could not be loaded.')
    }
    try {
      instrumentMidiPortamentos.value = await projectsApi.instrumentMidiPortamentos()
      activityLog.write('info', 'instrument-midi-portamentos.load', 'Instrument MIDI portamento controllers loaded.', {
        mappedCount: instrumentMidiPortamentos.value.assignments.filter(item => item.applicable).length,
        notApplicableCount: instrumentMidiPortamentos.value.assignments.filter(item => !item.applicable).length,
      })
    } catch (error) {
      instrumentMidiPortamentos.value = null
      activityLog.write('warning', 'instrument-midi-portamentos.load', error instanceof Error ? error.message : 'Instrument MIDI portamento controllers could not be loaded.')
    }
    await refreshBrowserRecovery()
...
function midiProgramLabel(instrumentId: string) {
  const assignment = instrumentMidiPrograms.value?.assignments.find(item => item.instrumentId === instrumentId)
...
function midiControllerLabel(instrumentId: string) {
  ...
  if (!assignment) return ''
  if (!assignment.applicable || assignment.controllerNumber == null || !assignment.controllerName) return 'Dynamics MIDI does not apply.'
  return `Dynamics MIDI → CC ${assignment.controllerNumber} ${assignment.controllerName}`
}
function midiPitchBendLabel(instrumentId: string) {
  const assignment = instrumentMidiPitchBends.value?.assignments.find(item => item.instrumentId === instrumentId)
  if (!assignment) return ''
  if (!assignment.applicable || assignment.rangeSemitones == null) return 'Pitch bend does not apply.'
  return `Pitch bend → ±${assignment.rangeSemitones} semitones`
}
function midiPortamentoLabel(instrumentId: string) {
  const assignment = instrumentMidiPortamentos.value?.assignments.find(item => item.instrumentId === instrumentId)
  if (!assignment) return ''
  if (!assignment.applicable || assignment.controllerNumber == null || !assignment.controllerName) return 'Portamento MIDI does not apply.'
  return `Portamento MIDI → CC ${assignment.controllerNumber} ${assignment.controllerName}`
}
    ...
    <template v-else-if="project">
      ...
      <section v-if="!phoneCaptureMode" id="arrangement-blueprint" class="arrangement-blueprint" aria-labelledby="arrangement-title">
        ...
        <section id="instrument-knowledge" class="instrument-knowledge" aria-labelledby="instrument-knowledge-title">
          ...
          <article v-for="instrument in instrumentProfiles?.instruments ?? []" :key="instrument.id" class="instrument-profile" :aria-label="`${instrument.name} instrument profile`">
            ...
            <p v-if="midiProgramLabel(instrument.id)">{{ midiProgramLabel(instrument.id) }}</p>
            <p v-if="midiControllerLabel(instrument.id)">{{ midiControllerLabel(instrument.id) }}</p>
            <p v-if="midiPitchBendLabel(instrument.id)">{{ midiPitchBendLabel(instrument.id) }}</p>
            <p v-if="midiPortamentoLabel(instrument.id)">{{ midiPortamentoLabel(instrument.id) }}</p>
        ...
        <p class="arrangement-boundary">Musical parts connect your approved notes to an arrangement purpose. Naming a catalog instrument is optional and reversible. It does not retarget notes or persist a cello or guitar sketch. Named pitched parts emit an inspectable General MIDI program on export; drum kit does not.</p>
      </section>
      ...
      <section v-if="!phoneCaptureMode" id="instrument-performance-retarget" class="instrument-performance-retarget" aria-labelledby="instrument-performance-retarget-title">
        <div>
          ...
          <p>Preview the same approved swell, slide, or onset on every catalog instrument, then store what applies onto a musical part that already names that instrument. Loudness gestures become swells; pitch gestures become slides only where the catalog map allows; onset gestures become kit hits. Piano, bass, flute, clarinet, trumpet, and synth pad do not take slides; drum kit does not take swell or slide; pitched instruments do not take kit hits. Violin swell is bow expression; flute swell is breath; clarinet and trumpet swells are legato. Synth pad swell is pad; synth lead swell is filter and synth lead slide is portamento; electric guitar swell is distortion and electric guitar slide is bend. Kit hits use General MIDI Acoustic Bass Drum (C2) instead of a melodic C4; the host does not choose snare or hat. Timing uses the take’s song placement plus take-relative milliseconds at the first tempo. Out-of-range slide pitches are skipped, not transposed. MIDI does not choose a renderer. MIDI is format 1 with named tracks. Stored musical-part labels export as MIDI instrument names on the tracks that actually receive those notes. The conductor track includes the song key as a MIDI key signature, each stored section title as a MIDI marker, each placed syllable as a MIDI lyric, each stored harmony chord as MIDI text, each stored breath after a placed syllable as a MIDI cue point, the stored artist name as a MIDI copyright notice when an artist is set, and the stored song description as MIDI text when a description is set, and each decided song role as MIDI text at that section’s start. Named pitched parts export on inspectable MIDI channels with an inspectable General MIDI program. Tagged dynamics use the instrument’s inspectable controller; flute swell is CC 2 and synth lead swell is CC 74; other catalog swells stay CC 11. Cello, violin, acoustic guitar, and electric guitar declare ±2 semitones of pitch-bend range without moving the wheel; synth-lead portamento is CC 65 and stays off so stored notes stay discrete. Drum kit stays on channel 10 without a program change, a dynamics controller, or a pitch-bend range; unassigned notes stay on channel 1; untagged dynamics stay CC 11.</p>
        </div>
        ...
        <article v-for="asset in instrumentRetargetTakes" :key="asset.id" class="harmony-note-sketch" :aria-label="`Catalog instrument retarget for ${asset.name}`">
          ...
          <div v-if="instrumentPerformanceSketches[asset.id]" class="harmony-note-sketch-result">
            <p>
              ...
              <span>Uses the first tempo only. Each instrument keeps its own catalog technique. Piano, bass, flute, clarinet, and trumpet slides stay unused. Drum-kit swell and slide stay unused. Pitched instruments do not take kit hits. Kit hits use Acoustic Bass Drum at C2. Named pitched parts use inspectable MIDI channels and an inspectable General MIDI program on named format-1 tracks. Stored musical-part labels export as MIDI instrument names on those tracks when the part actually contributes notes. The conductor track includes the song key as a MIDI key signature, each stored section title as a MIDI marker, each placed syllable as a MIDI lyric, each stored harmony chord as MIDI text, each stored breath after a placed syllable as a MIDI cue point, the stored artist name as a MIDI copyright notice when an artist is set, and the stored song description as MIDI text when a description is set, and each decided song role as MIDI text at that section’s start. Tagged dynamics use the instrument’s inspectable controller; flute swell is CC 2 and synth lead swell is CC 74; other catalog swells stay CC 11. Cello, violin, acoustic guitar, and electric guitar declare ±2 semitones of pitch-bend range without moving the wheel; synth-lead portamento is CC 65 and stays off so stored notes stay discrete. Drum kit stays on channel 10 without a program change, a dynamics controller, or a pitch-bend range; unassigned notes stay on channel 1; untagged dynamics stay CC 11.</span>
            </p>
