...
<template>
  <details class="vocal-chain" @toggle="event => { if (!(event.target as HTMLDetailsElement).open) discard() }">
    ...
    <p>This plays the original take through the low-cut, level-control, saturation, character-compression, cleanup, sibilance, and space settings already accepted for it, in the current production-job order. Jobs without a processor stay in the plan and are not heard.</p>
    <p>{{ description }}</p>
