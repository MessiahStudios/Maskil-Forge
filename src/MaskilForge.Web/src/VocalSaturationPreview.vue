...
import { prepareVocalSaturation, vocalSaturation, type VocalSaturationComparison } from './vocalSaturation'
...
const comparison = shallowRef<VocalSaturationComparison | null>(null)
...
const accepted = computed(() => props.recipe?.processorId === vocalSaturation.processorId && props.recipe.sourceSha256 === props.asset.sha256)
let generation = 0
...
async function prepare() {
  ...
  try {
    const result = await prepareVocalSaturation(projectsApi.originalVocalTakeUrl(props.projectId, props.asset.id), props.asset, controller.signal)
...
<template>
  <details class="vocal-saturation" @toggle="event => { if (!(event.target as HTMLDetailsElement).open) discard() }">
    <summary>Compare saturation <span v-if="accepted">· Accepted</span></summary>
    <p>Saturation / Color · A fixed gentle curve, sample minus 0.15 times its cube. Louder moments pick up a little harmonic color and lose a little peak. Silence and quiet phrases stay put. This is not level control, a gate, or an equalizer.</p>
    <p>Only this treatment is previewed, from the original take. It does not include an accepted low-cut, level control, or any other production job.</p>
    <p v-if="!roleEnabled">Add Saturation / Color in “Choose and order production jobs” to try this treatment.</p>
    <p v-if="accepted">Accepted starting point: color amount 0.15. Prepare a comparison to hear it again. Use Save to keep accepted settings with the song.</p>
    <div class="saturation-actions">
      ...
      <button v-if="accepted" type="button" class="quiet" :disabled="busy || preparing" @click="emit('clear', asset.id)">Clear accepted saturation</button>
    </div>
    <p v-if="message" role="status">{{ message }}</p>
    <div v-if="comparison" class="saturation-comparison">
      ...
      <label>Saturation preview<audio ref="processedPlayer" controls preload="metadata" :src="comparison.processedUrl" :aria-label="`Saturation preview for ${asset.name}`" @playing="playing('processed', $event)"></audio></label>
      ...
      <button v-if="!accepted" type="button" :disabled="busy || !heardOriginal || !heardProcessed" @click="accept">Accept saturation settings</button>
      <p v-if="!accepted && (!heardOriginal || !heardProcessed)">Play both versions to enable acceptance.</p>
    </div>
    <p class="saturation-boundary">Your saved original stays unchanged. Discard removes the temporary comparison; clearing accepted settings is undoable. There are no advanced controls for this starting point.</p>
  </details>
</template>

<style scoped>
.vocal-saturation { padding: .85rem; border: 1px solid #54614d; border-radius: .4rem; background: #10150f; }
summary { cursor: pointer; font-weight: 650; padding: .4rem 0; }
p { color: #abb4a4; line-height: 1.5; font-size: .85rem; }
.saturation-actions { display: flex; flex-wrap: wrap; gap: .5rem; margin: .7rem 0; }
.saturation-comparison { display: grid; gap: .6rem; }
label { display: grid; gap: .4rem; color: #e6cf91; }
audio { width: 100%; }
.saturation-boundary { border-top: 1px solid #394236; padding-top: .65rem; }
</style>
