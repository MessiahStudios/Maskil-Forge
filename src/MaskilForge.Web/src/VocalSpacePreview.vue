...
import { prepareVocalSpace, vocalSpace, type VocalSpaceComparison } from './vocalSpace'
...
const comparison = shallowRef<VocalSpaceComparison | null>(null)
...
const accepted = computed(() => props.recipe?.processorId === vocalSpace.processorId && props.recipe.sourceSha256 === props.asset.sha256)
let generation = 0
...
async function prepare() {
  ...
  try {
    const result = await prepareVocalSpace(projectsApi.originalVocalTakeUrl(props.projectId, props.asset.id), props.asset, controller.signal)
...
<template>
  <details class="vocal-space" @toggle="event => { if (!(event.target as HTMLDetailsElement).open) discard() }">
    <summary>Compare space <span v-if="accepted">· Accepted</span></summary>
    <p>Space · A reflection returns 80 ms later at 0.15 of the voice, with a 0.3 repeat behind it. The dry voice stays in front. If a reflection would pass available headroom, that reflection is eased and the dry sample stays. The preview ends with the take, so a reflection past the end is not added.</p>
    <p>Only this treatment is previewed, from the original take. It does not include an accepted low-cut, level control, saturation, character compression, cleanup, sibilance control, or any other production job.</p>
    <p v-if="!roleEnabled">Add Space in “Choose and order production jobs” to try this treatment.</p>
    <p v-if="accepted">Accepted starting point: 80 ms reflection · 0.15 level · 0.3 repeat. Prepare a comparison to hear it again. Use Save to keep accepted settings with the song.</p>
    <div class="space-actions">
      ...
      <button v-if="accepted" type="button" class="quiet" :disabled="busy || preparing" @click="emit('clear', asset.id)">Clear accepted space</button>
    </div>
    <p v-if="message" role="status">{{ message }}</p>
    <div v-if="comparison" class="space-comparison">
      ...
      <label>Space preview<audio ref="processedPlayer" controls preload="metadata" :src="comparison.processedUrl" :aria-label="`Space preview for ${asset.name}`" @playing="playing('processed', $event)"></audio></label>
      ...
      <button v-if="!accepted" type="button" :disabled="busy || !heardOriginal || !heardProcessed" @click="accept">Accept space settings</button>
      <p v-if="!accepted && (!heardOriginal || !heardProcessed)">Play both versions to enable acceptance.</p>
    </div>
    <p class="space-boundary">Your saved original stays unchanged. Discard removes the temporary comparison; clearing accepted settings is undoable. There are no advanced controls for this starting point.</p>
  </details>
</template>

<style scoped>
.vocal-space { padding: .85rem; border: 1px solid #54614d; border-radius: .4rem; background: #10150f; }
summary { cursor: pointer; font-weight: 650; padding: .4rem 0; }
p { color: #abb4a4; line-height: 1.5; font-size: .85rem; }
.space-actions { display: flex; flex-wrap: wrap; gap: .5rem; margin: .7rem 0; }
.space-comparison { display: grid; gap: .6rem; }
label { display: grid; gap: .4rem; color: #e6cf91; }
audio { width: 100%; }
.space-boundary { border-top: 1px solid #394236; padding-top: .65rem; }
</style>
