...
test('accepted chain steps follow plan order and ignore other jobs and other takes', () => {
  ...
  assert.equal(describeAcceptedVocalChain([]), 'Accept a low-cut or level-control treatment on this take to hear it here.')
})
...
test('chain preview rejects an empty plan and an unknown treatment', () => {
  ...
  assert.throws(() => renderAcceptedVocalChain(source, []), /one or both/)
  assert.throws(() => renderAcceptedVocalChain(source, [{ role: 'Space' }]), /low-cut and level control/)
