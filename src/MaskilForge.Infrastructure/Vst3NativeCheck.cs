...
public sealed record Vst3NativeComponentCheckRequest(string Location, string RelativePath, string ExpectedPlistSha256, string ClassId);
...
public sealed class Vst3NativeCheck(Vst3Discovery discovery)
{
    ...
    public async Task<Vst3NativeComponentCheckResult> InspectComponentAsync(Vst3NativeComponentCheckRequest request,
        CancellationToken cancellationToken = default)
    {
        var classId = request.ClassId ?? string.Empty;
        Vst3NativeComponentCheckResult Empty(string status) => new(status, "NotStarted", DateTimeOffset.UtcNow, classId, []);
        ...
        try
        {
            ...
            var outcome = await new Vst3ComponentProbeProcess().RunAsync(WorkerPath, bundle, binary, classId, cancellationToken);
            ...
            return new(status, outcome.LastCompletedStage, DateTimeOffset.UtcNow, outcome.ClassId, outcome.Buses,
                source, before, declaration.Sha256, outcome.ExitCode);
        }
