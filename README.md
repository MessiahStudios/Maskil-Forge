...
Undo and redo history is currently session-only. Section edits, manual phrase split/join actions, syllable stress decisions, and prosodic-weight decisions participate in that history. Redo restores the same phrase, pattern, and prosodic-unit identities; stress and prosody undo restore the exact prior values and provenance. Saved project content survives closing and reopening, but the command history itself does not.
...
Schema-v1 project files and recovery snapshots migrate in memory through schemas v2, v3, v4, v5, v6, and v7 when loaded. Their existing tempo, time signature, section identifiers, line identifiers, text, and section order are retained. Migrated sections receive an initial eight-bar placement, existing lyric words receive deterministic identifiers, schema-v3 syllables retain their IDs while receiving ordered positions and manual provenance, schema-v4 lines receive deterministic punctuation IDs and one default phrase, schema-v5 syllables begin with no stress annotation, and schema-v6 phrases begin with no prosodic pattern. Migration never invents weight decisions. The original file is not rewritten until the artist explicitly saves it.
