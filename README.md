...
The schema-v3 lyric-document foundation tokenizes structured lyric lines into individually addressable words while preserving punctuation in the original line text. Unchanged words retain their identifiers when nearby words are inserted or removed. Schema v4 adds ordered syllable entities with stable IDs and `Manual`, `Analyzer`, or `Imported` provenance. The current editor lets the artist enter or correct syllable boundaries directly; artist edits are stored as authoritative manual data. Maskil Forge does not automatically infer syllable boundaries, stress, breath points, rhythm, or musical placement yet.
...
Schema-v1 project files and recovery snapshots migrate in memory through schemas v2, v3, and v4 when loaded. Their existing tempo, time signature, section identifiers, line identifiers, text, and section order are retained. Migrated sections receive an initial eight-bar placement, existing lyric words receive deterministic identifiers, and schema-v3 syllables retain their IDs while receiving ordered positions and manual provenance. The original file is not rewritten until the artist explicitly saves it.
