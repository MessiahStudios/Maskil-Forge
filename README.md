...
The prototype and editable-demo MVP are complete through slice 5.15. Current named work is **Milestone 8.2**. Schema is **v31**. The instrument catalog is **version 4**. Desktop Music can switch between instrument-aware built-in previews and a user-supplied SF2, SF3, or DLS General MIDI bank, then export stored song facts as a format-1 Standard MIDI File. Phone Music stays hidden. Bundled sound banks, offline WAV rendering, plugin hosting, and Milestone 9.1 vocal-production intent are not started.
