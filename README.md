...
Slice 7.25 emits each stored syllable placement as a MIDI lyric on the conductor track at that syllable's song tick. Unplaced words and rhythm-candidate ghosts are omitted rather than invented. The host does not emit generic text events or cue points. Schema stays at v31. Catalog stays at version 4. Phone Music remains hidden.

Slice 7.26 emits each stored section harmony chord as a MIDI text event on the conductor track at that chord's start tick. The text is the stored chord symbol. Songs without an active progression omit chord text rather than inventing one. Named harmony options stay off the file. The host does not emit cue points or dump the raw lyric sheet. Schema stays at v31. Catalog stays at version 4. Phone Music remains hidden.

Slice 7.27 emits each stored breath after a placed syllable as a MIDI cue point on the conductor track at that syllable's song tick. Unplaced breaths and rhythm-candidate ghosts are omitted rather than invented. The visualization offset used to draw a breath after a lyric is not exported as musical time. Schema stays at v31. Catalog stays at version 4. Phone Music remains hidden.

Slice 7.28 hardens MIDI text interchange for artist-authored names and lyrics. ASCII metadata remains byte-for-byte compatible, while non-ASCII conductor-track names, section markers, and placed-syllable lyrics are encoded as strict UTF-8 instead of being silently stripped. Every text payload uses the Standard MIDI File variable-length size, including payloads longer than 127 bytes. Control characters stay off the file, metadata remains bounded to 80 Unicode scalar values, and the Song Graph remains the authoritative text when a legacy reader cannot display UTF-8. Schema stays at v31. Catalog stays at version 4. Phone Music remains hidden.

Slice 7.29 carries that language safety through the browser download boundary. MIDI and portable-project filenames preserve normalized Unicode text such as `Canción 夜` instead of degrading it to ASCII fragments. Path-unsafe punctuation becomes a separator, control and formatting characters stay out, Windows-reserved stems receive a safe prefix, and the filename stem is bounded by both Unicode scalar count and UTF-8 size. The rule changes only the suggested download name; it does not rename the project or alter exported bytes. Schema stays at v31. Catalog stays at version 4. Phone Music remains hidden.

Slice 7.30 keeps every MIDI track open through the current stored song-form boundary. A final eight-bar section beginning at bar 57 therefore carries the file through bar 65 instead of ending at that section marker. This is the current arrangement plan, not a duration inferred from lyrics or a promise about the final recording: section lengths remain artist-editable planning data, syllable placements remain onset anchors, and approved notes retain their exact stored durations. Notes or controller events beyond the planned form extend their own tracks; songs without sections retain event-derived length. The exporter adds only trailing duration and does not invent notes, rests, lyrics, audio, or performance. Schema stays at v31. Catalog stays at version 4. Phone Music remains hidden.

Slice 7.31 emits the stored artist name as a MIDI copyright notice on the conductor track when that name is present. Songs with playable notes but no artist omit the notice rather than inventing an author. Genre, description, and the song title stay off that field. Schema stays at v31. Catalog stays at version 4. Phone Music remains hidden.

Slice 7.32 emits each stored musical-part label as a MIDI instrument name on the catalog or Unassigned track that actually receives that part's notes. Catalog track names stay the 7.22 instrument names. Notes with no part, duplicate labels on the same track, and parts whose notes export elsewhere omit a name rather than inventing one. Schema stays at v31. Catalog stays at version 4. Phone Music remains hidden.

Slice 7.33 emits the stored song description as MIDI text on the conductor track when that description is present. Songs with playable notes but no description omit the text rather than inventing a blurb. Genre, title, and raw lyrics stay off that field. Schema stays at v31. Catalog stays at version 4. Phone Music remains hidden.
...
Schema-v1 project files and recovery snapshots migrate sequentially in memory through the current schema v31. Early migrations retain tempo, meter, section and lyric identities, text, and section order while adding deterministic lyric structure and empty decision slots. Later migrations add registered voicings, arrangement intent and roles, playable notes, musical parts, section delivery and structural function, the asset manifest and take names, analyzer observations, artist reviews and corrections, approved gestures, take placements, expression curves, and optional catalog-instrument tags. Existing projects receive empty collections or explicit unassigned values; migration never invents a performance, note, correction, gesture, instrument, or production decision. The original file is not rewritten until the artist explicitly saves it.
