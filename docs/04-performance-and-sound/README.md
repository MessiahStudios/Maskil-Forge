...
Slice 7.17 exports named catalog parts on inspectable MIDI channels. Cello uses 2, later pitched instruments follow catalog order while skipping 10, and electric guitar uses 13. Drum kit stays on 10. Unassigned notes and untagged dynamics stay on 1. Tagged dynamics follow the named instrument. MIDI still does not emit a program change.

Slice 7.18 emits inspectable General MIDI program changes for those named pitched parts. Drum kit still has no program change. Unassigned notes still have none.

Slice 7.19 emits tagged dynamics on each instrument’s inspectable MIDI controller. Flute swell is Breath Controller (CC 2). Synth lead swell is Brightness (CC 74). Other catalog swells stay Expression (CC 11). Untagged dynamics stay CC 11. Drum kit still has no dynamics controller.

Slice 7.20 declares an inspectable pitch-bend range of ±2 semitones for cello and violin slides and for acoustic-guitar and electric-guitar bends. MIDI export emits that range as RPN 0 and does not move the pitch wheel. Synth-lead portamento is not pitch bend. Drum kit still has no range.

Slice 7.21 declares an inspectable Portamento controller for synth-lead slides. MIDI export emits CC 65 Off so stored notes stay discrete. The host does not turn portamento on or invent a glide between pitches.

Slice 7.22 exports a format-1 Standard MIDI File with a named conductor track and one named track for Unassigned and each catalog instrument that actually exports notes. Unused catalog instruments do not get a track.

Slice 7.23 emits the stored song key as a MIDI key signature on the conductor track. Conventional major and minor spellings map to sharps or flats; unusual spellings are omitted rather than invented.

Slice 7.24 emits each stored section title as a MIDI marker on the conductor track at that section's start tick. Songs without sections omit markers rather than inventing them.

Slice 7.25 emits each stored syllable placement as a MIDI lyric on the conductor track at that syllable's song tick. Unplaced words and rhythm-candidate ghosts are omitted rather than invented.

Slice 7.26 emits each stored section harmony chord as a MIDI text event on the conductor track at that chord's start tick. Named harmony options and empty progressions are omitted rather than invented.

Slice 7.27 emits each stored breath after a placed syllable as a MIDI cue point on the conductor track at that syllable's song tick. Unplaced breaths and the lyric-timeline visualization offset are omitted rather than invented.
...
Slice 7.6 previews cello and guitar from the same approved gestures using the catalog articulation map. Slice 7.8 can persist that preview onto a part that already names cello or guitar. Slice 7.9 adds piano, bass, and kit adapters from that same map: swell applies as piano strike or bass finger; slides and both kit gestures stay unused. Slice 7.10 maps onset gestures onto drum-kit Hit on a part that already names the kit. Slice 7.12 maps Wave 2 orchestration the same way: violin uses bow expression and slide, flute uses breath, clarinet and trumpet use legato, and none of them take kit hits. Slice 7.13 can name synth pad, synth lead, or electric guitar. Slice 7.14 maps Wave 3 production the same way: synth pad uses pad, synth lead uses filter and portamento, electric guitar uses distortion and bend, and none of them take kit hits. Slice 7.15 stores kit hits as General MIDI Acoustic Bass Drum (C2) rather than a melodic C4, without choosing a kit piece or a drum MIDI channel. Slice 7.16 exports named kit hits on MIDI channel 10 without a program change or channels for pitched instruments. Slice 7.17 assigns inspectable MIDI channels to named catalog parts without a program change. Slice 7.18 emits inspectable General MIDI program changes for named pitched parts, still without a drum-kit program or a chosen renderer. Slice 7.19 emits tagged dynamics on each instrument’s inspectable MIDI controller, still without inventing a kit swell. Slice 7.20 declares an inspectable pitch-bend range for cello, violin, acoustic guitar, and electric guitar without moving the pitch wheel or treating synth-lead portamento as a bend. Slice 7.21 declares synth-lead portamento as CC 65 and keeps it off so stored notes stay discrete. Slice 7.22 exports those parts as named format-1 MIDI tracks. Slice 7.23 emits the stored song key as a MIDI key signature. Slice 7.24 emits stored section titles as MIDI markers. Slice 7.25 emits placed syllables as MIDI lyrics. Slice 7.26 emits stored section harmony as MIDI chord-symbol text. Slice 7.27 emits stored breaths after placed syllables as MIDI cue points. Slice 7.31 emits the stored artist name as a MIDI copyright notice when that name is present. Slice 7.32 emits stored musical-part labels as MIDI instrument names on the tracks that actually receive those notes. Slice 7.33 emits the stored song description as MIDI text when that description is present. Slice 7.34 emits each decided section song role as MIDI text at that section's start. Slice 7.35 emits the stored song genre as MIDI text when that genre is decided. Accepted instrumental notes do not replace the artist's lead vocal.
...
Milestone 9.3 realizes Corrective Tone with one fixed 80 Hz high-pass biquad, using the [W3C Audio EQ Cookbook](https://www.w3.org/TR/audio-eq-cookbook/) coefficients at Q 0.7071067811865476. The browser verifies the source SHA-256 and length, decodes it, and creates temporary original/processed PCM16 WAV comparisons at the same level. Both versions must be played before the UI permits acceptance. Schema v34 stores an undoable per-take recipe tied to that source digest, never a replacement recording. Reload requires explicit preparation and playback again. Recipes survive save, recovery, duplication, and packages; removing Corrective Tone requires clearing accepted settings first. The filter may thin a low voice, and a preview exceeding available headroom is refused without automatic gain adjustment. Other processors, transport processing, and rendered vocal export remain later work.

Milestone 9.4 adds transient, explained production-job proposals from set vocal direction. The review exposes additions, removals, and order changes; accepted low-cut recipes keep their required Corrective Tone role. Artists can audition the available low-cut example before choosing the plan, while other jobs are explicitly labeled as plans without audio processing. Accepting a proposal stores the ordinary chain with undo/redo and rejects changed vocal direction, chain, recipes, or asset manifests. It does not accept take settings, interpret written notes, diagnose audio, or persist a profile identity. Schema remains v34.
...
Milestone 9.9 adds a temporary preview of those accepted treatments in the current job order. Low-cut and level control are applied from the original take; other planned jobs are not heard. Reordering the plan changes the preview because the two treatments do not commute. The listening pass stores nothing and does not replace either recipe. Schema remains v36. Milestone 9.10 later includes accepted saturation in that same listening pass.

Milestone 9.10 realizes Saturation / Color with processor `maskil.vocal.saturation.v1`. The fixed curve is sample minus 0.15 times its cube. It adds a little harmonic color to louder moments and never raises a sample. Quiet phrases and silence stay essentially unchanged. There is no makeup gain, gate, equalizer, compressor, or limiter, so the role stays distinct from Transparent Level Control. Desktop Music prepares an original/processed comparison from the immutable take; both versions must be played before the fixed color amount can be accepted. Schema v37 allows that recipe to share a take with an accepted low-cut and level control, one recipe per role. Removing the job or clearing the plan requires clearing the matching accepted settings first. Profile proposals retain the job when a take already stores it. The chain preview hears it in plan order. The comparison is not applied to transport or export, and there are no advanced controls in this slice.

Milestone 9.11 realizes Character Compression with processor `maskil.vocal.character-compression.v1`. The fixed starting point shapes a linked peak envelope above −24 dBFS at 4:1. A louder moment is followed immediately and released over 250 ms, then the loudest moment is restored to the original peak. Quieter phrases can come forward. Nothing is raised above that original peak, and no limiter is applied. Transparent Level Control remains the separate treatment that leaves quiet phrases unboosted. Desktop Music prepares an original/processed comparison from the immutable take; both versions must be played before the fixed settings can be accepted. Schema remains v37 because the recipe uses the existing per-role fields. Removing the job or clearing the plan requires clearing the matching accepted settings first. Profile proposals retain the job when a take already stores it. The chain preview hears it in plan order. The comparison is not applied to transport or export, and there are no advanced controls in this slice.

Milestone 9.12 realizes Cleanup with processor `maskil.vocal.cleanup.v1`. Gaps below −40 dBFS ease toward one quarter of their level over 200 ms, and a sung level returns over 10 ms. The gaps are lowered, not silenced, and nothing is boosted. Desktop Music prepares an original/processed comparison from the immutable take; both versions must be played before the fixed settings can be accepted. Schema remains v37 because the recipe uses the existing per-role fields. Removing the job or clearing the plan requires clearing the matching accepted settings first. Profile proposals retain the job when a take already stores it. The chain preview hears it in plan order. The comparison is not applied to transport or export, and there are no advanced controls in this slice.
...
Milestone 9.16 plays a one-bar count-in at the song tempo and meter before a rough vocal recording. Recording starts on the next downbeat. The clicks are heard and are not written into the take. Cancelling the count-in records nothing. The choice stays in the current tab. Schema remains v39.

Milestone 9.15 lets the artist name one saved original take as the lead vocal. Other takes stay in the song. The choice is explicit, undoable, and cleared when that take is removed. It does not mix, tune, punch in, comp, or replace the recording. Schema v39 stores the choice only after the artist makes it. Guide vocals, lyric highlighting, punch-in, comping, and timing feedback remain later work. Milestone 9.17 adds the melody check.

Milestone 9.14 realizes Space with processor `maskil.vocal.space.v1`. A reflection returns 80 ms later at 0.15 of the voice, with a 0.3 repeat behind it. The dry sample is never reduced. If a reflection would leave the available range, only that added reflection is eased. The preview keeps the take's length. Desktop Music prepares an original/processed comparison from the immutable take; both versions must be played before the fixed settings can be accepted. Schema v38 stores the delay, repeat, and reflection level. Removing the job or clearing the plan requires clearing the matching accepted settings first. Profile proposals retain the job when a take already stores it. The chain preview hears it in plan order. The comparison is not applied to transport or export, and there are no advanced controls in this slice.
...
Own the starting points. Keep the artist in control. Built-in treatments are enough to finish the vocal. The artist describes the desired sound, compares the original with the proposed treatment, and explicitly accepts or rejects it. An external plugin is an optional realization of the same job. When that plugin is missing, the job and the artist’s intent stay, the missing implementation is reported, and the built-in treatment is offered where one exists. A different sound is never substituted in silence. Milestone 9.6 extends the available realizations without changing that workflow.
