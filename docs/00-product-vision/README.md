...
- Own the starting points. Keep the artist in control. Maskil Forge provides understandable built-in vocal treatments. The artist describes the desired sound, compares the original with the proposed treatment, and explicitly accepts or rejects it. External plugins are optional realizations of those same jobs. A project stays usable when an optional plugin is missing: the job and the artist’s intent remain, the missing implementation is reported, and the built-in treatment is offered where one exists. A different sound is never substituted in silence.
...
The repository contains an executable songwriting MVP and its product documentation. Local project discovery, recovery, Trash, portable project interchange, raw and structured lyrics, manual syllable and prosody decisions, editable section timing, harmony and arrangement intent, approved playable notes, instrument-aware synthesized browser preview with a few overtones on the pitched guide voices, rough-vocal recording and review, narrow browser pitch/onset/loudness analysis, gesture-to-note sketches, an inspectable desired vocal result, and format-1 MIDI interchange are implemented. The current editor labels section bars as arrangement planning rather than lyric-derived or final performed duration.

Automatic lyric interpretation, generated rhythm candidates, syllable duration, rests and melisma, synchronized audio clips, persistent undo history, bundled or production-quality instrumental rendering, additional vocal processors, mixing, and AI direction remain planned. Desktop Music supports an ordered vocal-production plan, a previewable low-cut treatment, a previewable fixed level-control treatment, a previewable fixed saturation treatment, a previewable fixed character-compression treatment, a previewable fixed cleanup treatment, a previewable fixed sibilance treatment, and a previewable fixed space treatment. Each acceptance is explicit, per take, and preserves the original recording. One saved take can be named the lead vocal while the other takes stay in the song. A rough recording can begin with a one-bar count-in at the song tempo; those clicks are not saved with the take. A saved take can be checked against the written notes without tuning the recording. When the singer has stored a corrected pitch for an inaccurate frame, that correction is what the check compares, and the original measurement stays on the take. Before recording, the singer can hear the top written line as a rehearsal tone that is not saved and does not become the vocal. Accepted treatments can be heard together in the current job order without storing another recording. Desktop Music can use an artist-supplied SF2, SF3, or DLS bank for real sample-based General MIDI preview, but the bank remains device-local and tab-scoped. The installable web shell supports browser-owned offline lyric capture and view-only saved snapshots; complete offline project editing and device synchronization are not implemented.
