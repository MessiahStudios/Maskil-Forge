...
Project JSON is a durable creative file format rather than a disposable API payload. Schema v14 writes a numeric `schemaVersion`, string `id`, title, creation and modification timestamps, raw lyric draft, sections (including ordered `harmony` chords and named `harmonyCandidates`), tracks, timeline, structured lyric words, ordered syllables with provenance and optional stress marks, addressable punctuation, word-referencing phrases, optional prosodic patterns, optional syllable placements, optional rhythm candidates, optional breath points, optional creative locks, and a song-level `key` (`tonic`, `accidental`, `mode`). A harmony chord has a stable identity, a `ChordSymbol`, a section-relative start position, a bar duration, and provenance. A harmony candidate has stable candidate and event identities, a name, provenance, and an ordered alternative chord progression; applying one explicitly replaces authoritative harmony while preserving compatible chord identities. A creative lock has a stable identity, `LyricLine` or `PhraseRhythm` scope, the referenced line (and phrase when required), and provenance. A breath point has a stable identity, references one existing syllable ID as the syllable after which the artist intends to breathe, and records `Manual`, `Analyzer`, or `Imported` provenance. Absence means undecided; punctuation never invents a breath. A rhythm candidate belongs to one phrase, has a stable identity and label, and contains ordered, identified events that reference existing syllable IDs and section-relative `BeatPosition` values. Candidates are possibilities; applying one explicitly replaces that phrase's authoritative syllable placements while preserving compatible placement IDs. The timeline contains its resolution, tempo map, time-signature map, and section placements. Strongly typed creative identifiers serialize as strings. Derived lyric-timeline projections resolve those section-relative placements to absolute song ticks for editor display and optional rhythm-option comparison; they are not written into the project file. Deterministic theory helpers provide pitch classes, intervals, major/natural-minor scales, a small chord vocabulary, transposition, and derived Roman numerals without inventing progressions.

The schema-v1 reader accepts both numeric and original object-shaped schema versions (`{ "value": 1 }`) and supplies defaults for raw lyrics and timestamps when loading early files. Missing `createdUtc` / `lastModifiedUtc` values are filled from the file's last-write time so repeated loads keep a stable revision for stale-session checks; those defaults are written into the project JSON only on an explicit save. The v1-to-v2 migration moves the existing tempo and meter into their maps and creates ordered eight-bar placements using the original section IDs. The v2-to-v3 migration tokenizes existing lyric lines and derives deterministic word IDs from the preserved line ID, token order, and text. The v3-to-v4 migration preserves existing syllable IDs and text while adding contiguous zero-based positions and `Manual` provenance. The v4-to-v5 migration derives deterministic punctuation IDs from the preserved line and creates one default phrase referencing all existing word IDs. The v5-to-v6 migration adds an unmarked stress slot without inventing an artist decision or changing syllable identity. The v6-to-v7 migration adds an empty prosody slot to each phrase without deriving weight from stress or creating new creative identities. The v7-to-v8 migration adds an empty placement collection to each lyric line without assigning beats. The v8-to-v9 migration adds an empty rhythm-candidate collection without inventing alternatives or selecting a rhythm. The v9-to-v10 migration adds an empty breath-point collection without inventing inhale decisions. The v10-to-v11 migration adds an empty creative-lock collection without inventing protections. The v11-to-v12 migration adds a default C major key without inventing progressions or chord decisions. The v12-to-v13 migration adds an empty harmony collection to each section without inventing chords. The v13-to-v14 migration adds an empty harmony-candidate collection without inventing alternatives. Migrations run in memory; the source file is not rewritten until an explicit save. Recovery snapshots use the same migration path.

Derived voice-leading review reads the authoritative section harmony without changing project schema or creative state. It references stable chord identities and reports pitch-class continuity only; octave/register voicings and instrument assignments remain outside this foundation.
