...
The preview shows the current and proposed job order. Acceptance appends Transparent Level Control only when absent, keeps all existing jobs in order, and uses the ordinary undoable chain command. A source signature rejects changed evidence, reviews, corrections, takes, or plan state. Discard, collapse, project changes, and navigation remove the temporary review and stop its player. No guidance object or new schema field is persisted. Existing chain save/recovery and package paths carry accepted jobs. This slice does not process audio. Milestone 9.8 later adds the fixed level-control comparison. External processor substitution remains pending its hosting dependency.

### Milestone 9.8 — Built-in transparent level control

**Implemented:** Desktop Music can preview and accept a fixed Transparent Level Control treatment on a saved original take. Processor `maskil.vocal.level-control.v1` follows the peak of the linked channels with a 20 ms attack and 120 ms release, then eases levels above −18 dBFS at a 2:1 ratio. The computed gain stays at or below 1. Quiet phrases are not raised, and the preview applies no makeup gain, limiter, or low-cut. Mono and stereo takes up to one minute at 8–96 kHz use the same headroom refusal as Corrective Tone.

The artist prepares a comparison, plays the original and processed versions, and explicitly accepts or discards. Acceptance stores the fixed settings against that take's asset ID and SHA-256. A take may keep this recipe and an accepted low-cut at the same time; each role still has at most one recipe. Clearing either recipe is undoable and does not remove the other. Removing Transparent Level Control from the plan, or clearing the plan, requires clearing accepted level-control settings first. The same rule still applies to Corrective Tone. A profile proposal keeps Transparent Level Control when a take already stores it.

Schema v36 migrates earlier projects without changing existing recipes. Save, recovery, duplication, and asset-owning packages carry both recipes and leave original bytes unchanged. There is no advanced control, transport processing, rendered vocal export, or VST substitution in this slice. Phone production controls stay hidden. Instrument catalog remains version 4.
